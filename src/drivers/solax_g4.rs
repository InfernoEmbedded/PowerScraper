use crate::config::{MqttBrokerConfig, SolaxG4ModbusConfig};
...
pub async fn run_solax_g4_driver(
    ...
    let client_id = format!("powerscraper-g4-{}", inverter_name);
    ...
    if let Some(password) = config.installer_password {
        ...
        if let Ok(addr) = resolve_address(&hostname).await {
            if let Ok(mut ctx) = tcp::connect(addr).await {
                // Address 0x0000 is UnlockPassword in G4
                let _ = ctx.write_single_register(0x0000, password).await;
                *lock = Some(ctx);
            }
        }
    }
                                        ...
                                        *req_power_clone.lock().await = power;
                                        ...
                                        if let Some(ref mut ctx) = *lock {
                                            if power != 0 {
                                                ...
                                                let _ = ctx.write_multiple_registers(0x007E, &[low_word, high_word]).await;
                                            } else {
...
mod tests {
    ...
    #[test]
    fn test_parse_solax_g4_registers() {
